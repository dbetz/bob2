...
void BobInitFile(BobInterpreter *c)
{
    ...
    if (!(BobFileDispatch = BobEnterCPtrObjectType(BobGlobalScope(c),NULL,"File",methods,properties)))
...
static void EnterPort(BobInterpreter *c,char *name,BobStream **pStream)
{
    ...
    if (!(s = BobMakeIndirectStream(c,pStream)))
        BobInsufficientMemory(c);
    BobCheck(c,2);
    BobPush(c,BobMakeFile(c,s));
    BobPush(c,BobInternCString(c,name));
    BobSetGlobalValue(BobGlobalScope(c),BobTop(c),c->sp[1]);
    BobDrop(c,2);
}
