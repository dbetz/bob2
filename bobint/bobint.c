...
/* prototypes */
static BobValue ExecuteCall(BobScope *scope,BobValue fun,int argc,va_list ap);
static void Execute(BobInterpreter *c);
static void SaveInterpreterState(BobInterpreter *c,BobSavedState *s);
static void RestoreInterpreterState(BobInterpreter *c);
...
/* SaveInterpreterState - save the current interpreter state */
static void SaveInterpreterState(BobInterpreter *c,BobSavedState *s)
{
    s->globals = c->currentScope.globals;
    s->sp = c->sp;
    s->fp = c->fp;
    s->env = c->env;
    if ((s->code = c->code) != NULL)
        s->pcoff = c->pc - c->cbase;
    s->next = c->savedState;
    c->savedState = s;
}

/* RestoreInterpreterState - restore the previous interpreter state */
static void RestoreInterpreterState(BobInterpreter *c)
{
    BobSavedState *s = c->savedState;
    c->currentScope.globals = s->globals;
    c->sp = s->sp;
    c->fp = s->fp;
    c->env = s->env;
...
/* BobCallFunctionByName - call a function by name */
BobValue BobCallFunctionByName(BobScope *scope,char *fname,int argc,...)
{
    ...
    BobCPush(c,BobInternCString(c,fname));
    if (!BobGlobalValue(scope,BobTop(c),&fun))
        BobCallErrorHandler(c,BobErrUnboundVariable,BobTop(c));
    BobDrop(c,1);
...
/* BobSendMessage - send a message to an object */
BobValue BobSendMessage(BobInterpreter *c,BobValue obj,BobValue selector,int argc,...)
{
    ...
    /* reserve space for the object, selector, _next and arguments */
    BobCheck(c,argc + 3);

    /* push the object, selector and _next argument */
    BobPush(c,obj);
    BobPush(c,selector);
    BobPush(c,obj); /* _next */
    ...
    /* setup the call */
    if (Send(c,&BobTopCDispatch,argc + 2)) {
        BobPopUnwindTarget(c);
        BobPopSavedState(c);
        return c->val;
    }
...
/* BobSendMessageByName - send a message to an object by name */
BobValue BobSendMessageByName(BobInterpreter *c,BobValue obj,char *sname,int argc,...)
{
    ...
    /* push the object, selector and _next argument */
    BobPush(c,obj);
    BobPush(c,c->nilValue); /* filled in below */
    BobPush(c,obj); /* _next */
    ...
    c->sp[argc + 1] = BobInternCString(c,sname);
...
static void Execute(BobInterpreter *c)
{
    for (;;) {
        ...
        case BobOpCLOSE:
            ...
            c->val = BobMakeMethod(c,c->val,c->env,c->currentScope.globals);
            break;
        case BobOpEREF:
            i = *c->pc++;
            for (p2 = c->env; --i >= 0; )
        ...
        case BobOpGREF:
            off = *c->pc++;
            off |= *c->pc++ << 8;
            if (!BobGlobalValue(BobCurrentScope(c),BobCompiledCodeLiteral(c->code,off),&c->val))
                BobCallErrorHandler(c,BobErrUnboundVariable,BobCompiledCodeLiteral(c->code,off));
            break;
        case BobOpGSET:
            off = *c->pc++;
            off |= *c->pc++ << 8;
            BobSetGlobalValue(BobCurrentScope(c),BobCompiledCodeLiteral(c->code,off),c->val);
            break;
        case BobOpGETP:
            p1 = BobPop(c);
        ...
        case BobOpPUSHSCOPE:
            BobCheck(c,2);
            BobPush(c,c->currentScope.globals);
            BobPush(c,c->currentScope.globals);
            break;
        case BobOpNEWOBJECT:
...
static int Call(BobInterpreter *c,FrameDispatch *d,int argc)
{
    ...
    c->currentScope.globals = BobMethodGlobals(method);
    c->code = code;
    c->cbase = cbase;
    c->pc = pc;
...
static BobValue *CallCopy(BobInterpreter *c,BobFrame *frame)
{
    ...
    call->globals = BobCopyValue(c,call->globals);
    if (call->code)
