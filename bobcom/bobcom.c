...
BobCompiler *BobMakeCompiler(BobInterpreter *ic,long csize,long lsize)
{
    ...
    if ((c = (BobCompiler *)BobAlloc(ic,sizeof(BobCompiler))) == NULL)
        return NULL;
    
    /* allocate and initialize the code buffer */
    if ((c->codebuf = (unsigned char *)BobAlloc(ic,(size_t)csize)) == NULL) {
        BobFree(ic,c);
        return NULL;
    }
...
BobValue BobCompileExpr(BobScope *scope)
{
    ...
    /* make a closure */
    code = BobMakeMethod(ic,code,ic->nilValue,scope->globals);
